...
## 第四章 空间内力组合推导 (双工况包络)
### 4.1 工况1：满水运行 + 风荷载 (控制跨中强度)
* **竖向设计总荷载 $Q_{{y1}}$** = $1.2 G_k + 1.4 Q_k$ = **{lr.工况1_竖向_总计:.2f}** kN
* **水平设计总荷载 $Q_{{z1}}$** = $1.4 \\cdot \\psi_w \\cdot W_k \\cdot D \\cdot L$ = **{lr.工况1_水平荷载:.2f}** kN
* **跨中竖向弯矩 $M_{{y1}}$** = $Q_{{y1}} \\cdot L / 8$ = **{My1:.2f}** kN·m
* **总合成最大弯矩 $M_{{total,1}}$** = $\\sqrt{{M_{{y1}}^2 + M_{{z1}}^2}}$ = **{M_total1:.2f}** kN·m

### 4.2 工况2：空管状态 + 风荷载 (控制局部支座/抗拔)
* **竖向设计总荷载 $Q_{{y2}}$** = $1.0 G_k$ (有利系数取1.0) = **{lr.工况2_竖向_总计:.2f}** kN
* **水平设计总荷载 $Q_{{z2}}$** = $1.4 \\cdot W_k \\cdot D \\cdot L$ = **{lr.工况2_水平荷载:.2f}** kN
* **跨中竖向弯矩 $M_{{y2}}$** = $Q_{{y2}} \\cdot L / 8$ = **{My2:.2f}** kN·m
* **总合成最大弯矩 $M_{{total,2}}$** = $\\sqrt{{M_{{y2}}^2 + M_{{z2}}^2}}$ = **{M_total2:.2f}** kN·m
