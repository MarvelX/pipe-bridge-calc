...
## 第四章 宏观外部推力组合推导
*注：内水压力与温度作用直接在第五章管壁内部参与应力折算，不产生宏观支座推力。*

### 4.1 工况 1 (极端台风态)：恒载 + 风载 (无活载)
* **永久作用总和 $\\Sigma G$** = $G_{{pipe}} + G_{{water}} + G_{{anti}} + G_{{add}}$ = **{(lr.self_weight_kN + lr.water_weight_kN + lr.anti_corrosion_kN + lr.additional_kN):.2f}** kN
* **竖向设计总推力 $Q_{{y1}}$** = $1.2 \\Sigma G$ = **{lr.工况1_竖向_总计:.2f}** kN
* **水平设计总推力 $Q_{{z1}}$** = $1.4 F_{{qw}}$ = **{lr.工况1_水平荷载:.2f}** kN
* **跨中最大宏观弯矩 $M_{{total,1}}$** = $\\sqrt{{(Q_{{y1}}L/8)^2 + (Q_{{z1}}L/8)^2}}$ = **{M_total1:.2f}** kN·m

### 4.2 工况 2 (运行检修态)：恒载 + 活载 (无风载)
* **竖向设计总推力 $Q_{{y2}}$** = $1.2 \\Sigma G + 1.4 F_{{live}}$ = **{lr.工况2_竖向_总计:.2f}** kN
* **水平设计总推力 $Q_{{z2}}$** = 0.00 kN (本工况不计风载)
* **跨中最大宏观弯矩 $M_{{total,2}}$** = $Q_{{y2}} \\cdot L / 8$ = **{M_total2:.2f}** kN·m
